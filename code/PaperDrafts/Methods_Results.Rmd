...
The current study included 21 adults ($M_{age}= 34.05$, $SD_{age}= 14.41$, $range: 19-67$) with clinically significant emotional eating behaviors (defined as experiencing an emotional eating episode at least four times (e.g. at least once a week) over the last 28 days. Participants were required to be proficient in English and be able to commit to attending both in lab assessments as well as fulfill all the study requirements. The sample included 18 women (85.7% of the sample) and 3 men (14.3% of the sample). In the sample 61.9% identified as White/European American/Caucasian, 14.29% identified as multiracial, 9.52% identified as Asian American, 9.52% identified as Asian/Pacific-Islander, and 4.76% identified as “Other”. At baseline participants had an average BMI of 27.79 ($SD= 6.92$, $range= 20.30-49.30$) and reported an average of 34.39 ($SD= 6.92$, $range= 11-60$) emotional eating episodes in the past 28 days. The study was approved by the Institutional Review Board. 

##Procedures.

After completing a phone screen to confirm initial eligibility, participants attended an in-lab baseline assessment where they completed a series of self-report measures and where physiological signals were collected via the Empatica E4 device as participants completed behavioral tasks (the Mirror Tracing Persistence Task and the Paced Auditory Serial Addition Test) designed to induce frustration. Following completion of the baseline assessment, participants received instructions on how to use the Empatica E4 device and an EMA app [PACO app; @evans2016] at home for a four-week data collection period. During the data collection period, participants were asked to wear the Empatica E4 device every day during all waking hours. Participants were also asked to use the event button located on the E4 to record urges to eat due to their emotions (one button press) and record the occurrence of an emotional eating episode (two button presses). Participants were instructed to upload the data from their Empatica wristband to a Manager program that was installed on their computer at the end of every day. Participants were also instructed to respond to six EMA surveys which were distributed semi-randomly throughout the hours of 8am and 10pm via the PACO app during the data collection period. The EMA surveys assessed for the occurrence of emotional eating episodes and urges since the last survey that were not already reported via button push or a self-initiated EMA survey and assessed the presence or absence of 14 emotional states. Participants were asked to self-initiate an EMA survey whenever an emotional eating episode occurred, self-report surveys accounted for (6.86%) of total emotional eating episodes reported. Participants also filled out an additional survey at the end of every day which included questions about alcohol use and exercise.  After the four weeks, a post-study assessment was conducted. At the post-study assessment measures of emotion, emotional eating, and the tasks to induce emotion were again conducted. Participants also filled out a survey containing questions about the feasibility and acceptability of using the PACO app and wearing the Empatica E4 wristband.
...
```{r missingness-time, echo = FALSE, fig.cap='(ref:missingness-time)'}
df_time %>% 
  # use params threshold = 250, windows = 2
  filter(threshold == 250 & winds == 2) %>%
  # remove high stress controls
...
```{r missingness-freq, fig.cap='(ref:missingness-freq)', echo = FALSE}
df_freq %>% 
  # use params threshold = 100, windows = 2
  filter(threshold == 100 & winds == 2) %>%
  # remove high stress controls
...
The mean values for each time-domain feature are shown in Table \@ref(tab:timeSummary).
...
```{r timeComparisonPlot, echo = FALSE, warning = FALSE, message = FALSE, fig.cap = '(ref:timeComparisonPlot)'}
df_time_summaries %>%
  filter(str_detect(statistic, "control|episode|p.val")) %>%
  spread(key = statistic, value = value) %>%
  select(-p.val, everything()) %>%
...
A similar approach was taken to analyze the frequency domain. Table \@ref(tab:freqSummary) outlines the mean values of features.
...
```{r freqComparisonPlot, echo = FALSE, warning = FALSE, message = FALSE, fig.cap = '(ref:freqComparisonPlot)'}
...
(ref:freqComparisonPlotA) Between-Groups Permutation *t*-test of Frequency Domain Heart Rate Features

(ref:freqComparisonPlotB) Between-Groups Permutation *t*-test of High Frequency Features

(ref:freqComparisonPlotC) Between-Groups Permutation *t*-test of Low Frequency Features

(ref:freqComparisonPlotD) Between-Groups Permutation *t*-test of Low Frequency-High Frequency Ratio Features

```{r freqComparisonPlotA, echo = FALSE, warning = FALSE, message = FALSE, fig.cap = '(ref:freqComparisonPlotA)'}
df_freq_summaries %>%
  filter(str_detect(statistic, "control|episode|p.val")) %>%
  spread(key = statistic, value = value) %>%
  select(-p.val, everything()) %>%
...
```{r freqComparisonPlotD, echo = FALSE, warning = FALSE, message = FALSE, fig.cap = '(ref:freqComparisonPlotD)'}
df_freq_summaries %>%
  filter(str_detect(statistic, "control|episode|p.val")) %>%
  spread(key = statistic, value = value) %>%
  select(-p.val, everything()) %>%
...
```{r varImportance-time, echo = FALSE, warning = FALSE, message = FALSE, fig.cap = '(ref:varImportance-time)'}
time_domain_deltas %>% 
  group_by(variable) %>%
...
```{r varImportance-freq, echo = FALSE, warning = FALSE, message = FALSE, fig.cap = '(ref:varImportance-freq)'}
freq_domain_deltas %>% 
  group_by(variable) %>%
  summarise(Delta_Probability = mean(Delta_Probability)) %>%
...
In the frequency domain, the most important features by measure of decrease in model AUC were the High Frequency windows $HF_2$, $HF_6$, and $HF_3$. Increasing values in $HF_2$ by one unit tended to decrease the probability of observations being predicted as episode, suggesting that stronger signals in the high frequency band at this time in the window decrease the likelihood of an eating episode. This inference is the opposite, however, for $HF_6$ and $HF_3$, which show that increasing these variables' values by one unit tends to increase the likelihood of an eating episode. Interestingly, while the heart rate variables *Avg_HR*, *Avg_niHR*, and *End_niHR* had significantly different means, their affect on the achieved AUC was relatively minimal, indicating they had very small feature importance for this model.

(ref:highfreq-timeseries) Time Series of High Frequency Band

To better interpret how the signal in the high frequency band of heart rate varies through the 30-minute window, we plot these values in Figure \@ref(fig:highfreq-timeseries).

```{r highfreq-timeseries, echo = FALSE, warning = FALSE, message = FALSE, fig.cap = '(ref:highfreq-timeseries)'}
df_freq_ml %>%
